...
class BaseDataSet(Dataset):
    ...
    def _augmentation(self, image, label):
        h, w, _ = image.shape
        #print('aug', h, w, _) #2001 2001 4
        #print(label.shape)
        if self.base_size is not None:
            #print('base_size', self.base_size) # set in config file # removed now from config file
            image, label = self._resize(image, label)
        ...
        #print(self.mean)
        #print(self.std)
        #print(self.to_tensor(image).shape) #torch.Size([4, 320, 320])
