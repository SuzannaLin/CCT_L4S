...
class BaseTrainer:
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs+1):
            ...
            if self.mnt_mode != 'off' and epoch % self.config['trainer']['val_per_epochs'] == 0:
                try:
                    #print('current:', log[self.mnt_metric]) # current Mean_IoU
                    print('best metric:', self.mnt_best)
                    #print(log) # {'epoch': 2, 'val_loss': 0.13106, 'Pixel_Accuracy': 0.97, 'Mean_IoU': 0.575, 'Precision': 0.319, 'Recall': 0.265, 'f1': 0.261, 'Class_IoU': {0: 0.97, 1: 0.18}}
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        self.mnt_best = checkpoint['monitor_best'] /2 ## TEMPORARY
        self.not_improved_count = 0
