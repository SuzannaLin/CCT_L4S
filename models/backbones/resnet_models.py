...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=16, deep_base=False, norm_type=None):
        super(ResNet, self).__init__()
        self.inplanes = 128 if deep_base else 64
        print('change number of channels in resnet_models, and pretrained weigths in module_helper')
        if deep_base:
            #print('deepbase') # deepbase
            self.prefix = nn.Sequential(OrderedDict([
                ('conv1', nn.Conv2d(11, 64, kernel_size=3, stride=2, padding=1, bias=False)),
