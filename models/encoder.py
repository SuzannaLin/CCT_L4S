...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        #x:  torch.Size([2, 11, 128, 128])
        #print('x: ', x.shape) # x:  torch.Size([2, 2000, 2000, 4]) should be : x:  torch.Size([3, 3, 320, 320])
        x = self.base(x)
        x = self.psp(x)
        return x

    def get_backbone_params(self):
        #print('get backbone params')
        return self.base.parameters()

    def get_module_params(self):
        #print('get module params')
