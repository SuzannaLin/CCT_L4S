...
from utils.losses import CE_loss, softmax_mse_loss
...
class CCT(BaseModel):
    def __init__(self, num_classes, conf, sup_loss=None, cons_w_unsup=None, ignore_index=None, testing=False,
        ...
        # Create the model
        print('create the model with pretrained =',pretrained) # False
    ...
    def forward(self, x_l=None, target_l=None, x_ul=None, target_ul=None, curr_iter=None, epoch=None):
        ...
        #print('x_l.shape', x_l.shape) # torch.Size([2, 11, 128, 128])
        output_l = self.main_decoder(self.encoder(x_l))
        #print(output_l)
        #print('M output_l.shape',output_l.shape) # torch.Size([2, 2, 128, 128])
        #print('M target.shape',target_l.shape) # torch.Size([3, 128, 128])
        #print('input shape',x_l.shape,'output shape,',output_l.shape) # input shape torch.Size([2, 11, 128, 128]) output shape, torch.Size([2, 2, 128, 128])
        
        ### NO INTERPOLATION
        #if output_l.shape != x_l.shape:
        #    #print('biliniear interpolation')
        #    output_l = F.interpolate(output_l, size=input_size, mode='bilinear', align_corners=True)
        #print('after interpolation', output_l.shape,target_l.shape) # torch.Size([2, 2, 128, 128]),torch.Size([2, 128, 128])
        # Supervised loss
        #print(self.ignore_index, self.softmax_temp) # 255 1
        #print(self.sup_loss_w) # 1
        if self.sup_type == 'CE':
