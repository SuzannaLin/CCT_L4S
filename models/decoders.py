...
def upsample(in_channels, out_channels, upscale, kernel_size=3):
    ...
    #print(in_channels,out_channels,upscale) # 512, 2, 8
    layers = []
    ...
    for i in range(int(math.log(upscale, 2))):
        #print(i) # 0 , 1 , 2
        layers.append(PixelShuffle(out_channels, scale=2)) ## need to change this to improve checkerboard
    return nn.Sequential(*layers)

def upsample_de(in_channels, out_channels, upscale, kernel_size=3):
    # A series of x 2 upsamling until we get to the upscale we want
    #print(in_channels,out_channels,upscale,kernel_size) # 512, 2, 8, 3
    layers = []     # target [2, 320, 320], input [2, 2, 81, 81]
    conv1x1 = nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=False)
    nn.init.kaiming_normal_(conv1x1.weight.data, nonlinearity='relu')
    layers.append(conv1x1)
    for i in range(int(math.log(upscale, 2))):
        ...
        layers.append(relu)
        deconv = nn.ConvTranspose2d(out_channels, out_channels, kernel_size=4, stride=2)
        
        nn.init.kaiming_normal_(deconv.weight.data, nonlinearity='relu')
        layers.append(deconv)
    return nn.Sequential(*layers)

def upsample_in(in_channels, out_channels, upscale, kernel_size=3):
    # A series of x 2 upsamling until we get to the upscale we want
    #print(in_channels,out_channels,upscale,kernel_size) # 512, 2, 8, 3
    layers = []
    conv1x1 = nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=False) # squeezing all 512 channels
    nn.init.kaiming_normal_(conv1x1.weight.data, nonlinearity='relu')
    layers.append(conv1x1)
    #relu = nn.ReLU(inplace=True),
    #layers.append(relu)
...
class MainDecoder(nn.Module):
    ...
    def forward(self, x):
        #print(x.shape) # torch.Size([3, 512, 16, 16])
        #('1',x.shape)
        x = self.upsample(x)
        #print('2',x.shape) # torch.Size([3, 2, 142, 142])
        #size = x.shape[-1]
        #x = F.interpolate(x, size=size*self.upscale, mode='bilinear', align_corners=False)
        #print('3',x.shape) # torch.Size([3, 2, 128, 128])
...
class DropOutDecoder(nn.Module):
    ...
    def forward(self, x, _):
        ...
        size = x.shape[-1] # added
        #x = F.interpolate(x, size=size*self.upscale, mode='bilinear', align_corners=False) # added
        return x
...
class ContextMaskingDecoder(nn.Module):
    ...
    def forward(self, x, pred=None):
        ...
        #size = x_masked_context.shape[-1] # added
        #x_masked_context = F.interpolate(x_masked_context, size=size*self.upscale, mode='bilinear', align_corners=False) # added
        return x_masked_context
...
class ObjectMaskingDecoder(nn.Module):
    ...
    def forward(self, x, pred=None):
        ...
        #size = x_masked_obj.shape[-1] # added
        #x_masked_obj = F.interpolate(x_masked_obj, size=size*self.upscale, mode='bilinear', align_corners=False) # added
        return x_masked_obj
