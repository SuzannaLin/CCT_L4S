...
class HTML:
    ...
    def add_results(self, epoch, seg_results, width=400, domain=None):
            ...
            with tr():
                with td(style="word-wrap: break-word;", halign="center", valign="top"):
                    ...
                    if domain is not None:
                        td(f'Mean_IoU_{domain} : {np.round(seg_results[f"Mean_IoU_{domain}"],6)}')
                        td(f'PixelAcc_{domain} : {seg_results[f"Pixel_Accuracy_{domain}"]}')
                        td(f'Val Loss_{domain} : {seg_results[f"val_loss_{domain}"]}')
                    else:
                        td(f'Mean_IoU : {np.round(seg_results["Mean_IoU"],5)}')
                        td(f'Class_IoU : {seg_results["Class_IoU"]}') # added this
                        td(f'F1 : {seg_results["f1"]}')
                        td(f'Precision : {seg_results["Precision"]}')
                        td(f'Recall : {seg_results["Recall"]}')
                        td(f'PixelAcc : {seg_results["Pixel_Accuracy"]}')
                        td(f'Val Loss : {seg_results["val_loss"]}')
